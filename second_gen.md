...
class MyPhysicsGame extends Forge2DGame {
  ...
  FutureOr<void> onLoad() async {
    ...
    camera.viewfinder.zoom = 7.5; // Set the desired zoom level here
    return super.onLoad();
  }
  ...
  Future<void> addBackground() async {
    await world.add(Background(
      ...
      repeatY: 5,
      offsetX: -250,
      offsetY: -100, // Start the background 50 pixels to the left
    ));
  }
  ...
  Future<void> addGround() {
    ...
    return world.addAll([
      for (var x = camera.visibleWorldRect.left - groundOffset;
          ...
          x += tileWidth - overlap) // Subtract overlap from tile width
        Ground(
